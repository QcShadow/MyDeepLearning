...
from model.CBAMNet import CBAMNet
...
writer = SummaryWriter('/root/autodl-tmp/project/logs_train/logs_CBAMNet')
...
if best_model_state_dict is not None:
    torch.save(best_model_state_dict, '/root/autodl-tmp/project/model_test/best_CBAMNet.pth')
    print(f"最佳模型已保存,该模型在epoch+1={best_model_number}时取得,在验证集上的准确率为{best_accuracy}")


torch.save(model.state_dict(), '/root/autodl-tmp/project/model_test/final_CBAMNet.pth')
print(f"最终模型已保存, 在验证集上的准确率为{accuracy}%")

writer.close()

#启动tensorboard命令：python -m tensorboard.main --logdir=/root/autodl-tmp/project/logs_train/logs_CBAMNet