...
# 创建 TensorBoard 日志
writer = SummaryWriter('/root/autodl-tmp/project/logs_train/logs_ResNet50again')
...
if best_model_state_dict is not None:
    torch.save(best_model_state_dict, '/root/autodl-tmp/project/model_test/best_ResNet50again.pth')
    print(f"模型已保存,该模型在epoch={best_model_number}时取得,在验证集上的准确率为{best_accuracy}")

torch.save(model.state_dict(), '/root/autodl-tmp/project/model_test/final_ResNet50again.pth')
print(f"最终模型已保存, 在验证集上的准确率为{accuracy}%")

writer.close()

# 启动tensorboard命令：python -m tensorboard.main --logdir=logs_train/logs_ResNet50