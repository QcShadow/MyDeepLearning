...
from model.CNNTransformer import CNNTransformer
...
# 初始化模型
model = CNNTransformer(input_dim=512, num_heads=8, hidden_dim=256, num_layers=2).to(device)
...
writer = SummaryWriter('/root/autodl-tmp/project/logs_train/logs_CNNTransformer')
...
if best_model_state_dict is not None:
    torch.save(best_model_state_dict, '/root/autodl-tmp/project/model_test/best_CNNTransformer.pth')
    print(f"最佳模型已保存,该模型在epoch+1={best_model_number}时取得,在验证集上的准确率为{best_accuracy}")


torch.save(model.state_dict(), '/root/autodl-tmp/project/model_test/final_CNNTransformer.pth')
print(f"最终模型已保存, 在验证集上的准确率为{accuracy}%")

writer.close()

#启动tensorboard命令：python -m tensorboard.main --logdir=/root/autodl-tmp/project/logs_train/logs_CNNTransformer